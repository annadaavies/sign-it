    ...
    def backward_pass(self, gradient_values: numpy.ndarray):
        ...
        if self.weight_regulariser_l2 > 0:
            self.gradient_weights += (
                2 * self.weight_regulariser_l2 * self.weights
            )  # Calculation based on the derivative of L2 regularisation.

        if self.bias_regulariser_l1 > 0:
            gradient_l1 = numpy.ones_like(
                self.biases
            )  # Same as described above for weight gradients, but for bias gradients.
        ...
        if self.bias_regulariser_l2 > 0:
            self.gradient_biases += (
                2 * self.bias_regulariser_l2 * self.biases
            )  # Calculation based on the derivative of L2 regularisation.

        self.gradient_inputs = numpy.dot(
            gradient_values, self.weights.T
        )  # The gradient of the inputs is the dot product of the gradient valuse and the transposed weights. The weights must be transposed as you want to output to be the same shape as the gradient from the previous layer.
...
class DropoutLayer:
    ...
    def __init__(self, rate):
        self.rate = (
            1 - rate
        )  # We invert the inputted rate. For example, for a dropout rate of 0.1, you need a success rate of 0.9.
    ...
    def forward_pass(self, inputs, training):
        ...
        self.binary_mask = (
            numpy.random.binomial(1, self.rate, size=inputs.shape) / self.rate
        )  # np.random.binomial(n, p, size). n (number of trials), p (probability of success of the experiment), size (additional parameter size).
...
class LinearActivation:
    ...
    def forward_pass(self, inputs, training: bool):
        self.inputs = inputs
        self.output = inputs
...
class ReluActivation:
    ...
    def forward_pass(self, inputs: numpy.ndarray, training: bool):
        ...
        self.output = numpy.maximum(
            0, inputs
        )  # This creates an output matrix where all the inputs bigger than or equal to zero keep their value, where all the inputs less than 0 become 0 (see reLU activation section in notebook for further info.).
    ...
    def backward_pass(self, gradient_values: numpy.ndarray):
        ...
        self.gradient_inputs = (
            gradient_values.copy()
        )  # This gradient values will need to be modified, so a copy is made to ensure that changes are only made to gradient inputs and not gradient values.
        self.gradient_inputs[self.inputs <= 0] = (
            0  # When calculating the gradients of the ReLU function, all inputs less than or equal to zero should have a gradient of 0.
        )
...
class SoftmaxActivation:
    ...
    def forward_pass(self, inputs: numpy.ndarray, training):
        ...
        exp_values = numpy.exp(
            inputs - numpy.max(inputs, axis=1, keepdims=True)
        )  # This essentially subtracts the max value of each row of the inputs. As explored in more detail in notebook, this prevents dead neurons and exploding values.
        summed_exp_values = numpy.sum(
        ...
        probabilities = (
            exp_values / summed_exp_values
        )  # This divides each exponential neuron value by the corresponding sample exponential sum (each row being a sample). It is important to keep the same dimensions as before for probabilities so that you eventually get a weighted probability for each output class.
    ...
    def backward_pass(self, gradient_values: numpy.ndarray):
        ...
        """
        self.gradient_inputs = numpy.empty_like(
            gradient_values
        )  # This creates an empty array - which will become the gradient array - that has the same shape as the gradients received with respect to the outputs of the activation function (that need to have chain rule applied to get back to gradients with respect to inputs).
        for index, (output, gradient_value) in enumerate(
            zip(self.output, gradient_values)
        ):  # This will iterate through self.output and predicted_values simulataneously. zip() pairs each output from the current layer with its corresponding gradient. Enumerate keeps track of the index of the current iteration and the paired values.
            output = output.reshape(
                -1, 1
            )  # This reshapes each output into a column vector (hence the '1' parameter) with an unspecified number of rows.
            jacobian_matrix = numpy.diagflat(output) - numpy.dot(
                output, output.T
            )  # Calculating the partial derivatives (for the Jacobian matrix) using the formula.
            self.gradient_inputs[index] = numpy.dot(
                jacobian_matrix, gradient_value
            )  # Calculating the final product of the Jacobian matrix and the gradient vector (from the passed in gradient array), storing the resulting vector as a row in the empty gradient_inputs array.
...
class Loss:
    ...
    def regularisation_loss(self):
        ...
        for layer in self.trainable_layers:
            ...
            if layer.weight_regulariser_l1 > 0:
                regularisation_loss += layer.weight_regulariser_l1 * numpy.sum(
                    numpy.abs(layer.weights)
                )  # This performs the L1 regularisation calculation. Multiplying the sum of the absolute values of the weights by the L1's weight penalty value.

            if layer.weight_regulariser_l2 > 0:
                regularisation_loss += layer.weight_regulariser_l2 * numpy.sum(
                    layer.weights * layer.weights
                )  # This performs the L2 regularisation calculation. Multiplying the sum of the squares of the weights by the L2's weight penalty value.

            if layer.bias_regulariser_l1 > 0:
                regularisation_loss += layer.bias_regulariser_l1 * numpy.sum(
                    numpy.abs(layer.biases)
                )  # This performs the L1 regularisation calculation. Multiplying the sum of the absolute values of the biases by the L1's bias penalty value.

            if layer.bias_regulariser_l2 > 0:
                regularisation_loss += layer.bias_regulariser_l2 * numpy.sum(
                    layer.biases * layer.biases
                )  # This performs the L2 regularisation calculation. Multiplying the sum of the squares of the biases by the L2's bias penalty value.

        return regularisation_loss

    def remember_trainable_layers(
        self, trainable_layers
    ):  # This method essentiall 'tells' the loss object which layers in the Model object are trainable.
        self.trainable_layers = trainable_layers

    def calculate(
        self,
        output: numpy.ndarray,
        true_prediction_values: numpy.ndarray,
        ...
        """
        sample_losses = self.forward_pass(
            output, true_prediction_values
        )  # The forward pass is called to make sample_losses, which is a 1D array where each value in the array is the log confidence value for a sample.
        data_loss = numpy.mean(
            sample_losses
        )  # This finds the mean loss value for all the samples.

        self.accumulated_sum += numpy.sum(
            sample_losses
        )  # Need to add this to calculate the sample-wise average. Mathematically, you can just sum the losses from all epoch batches and counts to calculate the mean value at the end of each epoch.
    ...
    def calculate_accumulated(self, regularisation=False):
        """
        ...
        )  # Regularisation loss does not need to be accumulated as it's calculated from the current state of layer parameters.

    def reset_epoch(
        self,
    ):  # This method is to reset variables for accumulated loss each time you start a new epoch (i.e. each time you pass through the training data again).
...
class CategoricalCrossEntropyLoss(Loss):
    ...
    def forward_pass(
        self, predicted_values: numpy.ndarray, true_prediction_values: numpy.ndarray
    ) -> numpy.ndarray:
        """
        ...
        if (
            len(true_prediction_values.shape) == 1
            ...
            correct_confidences = predicted_values_adjusted[
                range(sample_number), true_prediction_values
            ...
            len(true_prediction_values.shape) == 2
        ):  # If the true prediction values are two dimensional, then it is using a set of one-hot encoded vectors.
            correct_confidences = numpy.sum(
                predicted_values_adjusted * true_prediction_values, axis=1
            )  # This is a similar technique as before. Instead of filtering confidences based on the true prediction values, you multiply the confidence by the targets, zeroing out all the ones except the confidences for the correct result for each sample. You then perform a sum along the rows to preserve the number of rows (and, therefore, the number of samples) but get it to the point where there is one confidence value per row (and, therefore, one confidence value per sample) which is the confidence for that sample's correct

        negative_log_confidences = -numpy.log(
            correct_confidences
        )  # This follows the equations for confidence described further in notebook.

        return negative_log_confidences  # Returns the losses for each class, for each sample.

    def backward_pass(
        self, gradient_values: numpy.ndarray, true_prediction_values: numpy.ndarray
    ):
        """
        ...
        samples = len(
            gradient_values
        )  # This finds the number of samples (as this is the number of rows of the predicted_values matrix).
        labels = len(
            gradient_values[0]
        )  # This finds the number of classes (finding the number of columns in the first row as the number of output values/classes should be the same for each sample).

        if (
            len(true_prediction_values.shape) == 1
        ):  # If the true prediction values are only one dimension (indicating that the labels are one-hot vectors/sparse vectors)
            true_prediction_values = numpy.eye(labels)[
                true_prediction_values
            ]  # This converts it into a one-dimensional matrix with the row that the correct label is on. e.g. if the correct label is 1 and there are three labels: [0,1,0]

        self.gradient_inputs = (
            -(true_prediction_values) / gradient_values
        )  # This calculates the gradient based on the derivative of the categorical cross entropy loss function, using the relevant mathematical equation.
        self.gradient_inputs = (
            self.gradients / samples
        )  # This noramlises the gradient based on the number of samples
...
class BinaryCrossEntropyLoss(Loss):
    ...
    def forward_pass(self, predicted_values, true_prediction_values):
        ...
        sample_losses = -(
            true_prediction_values * numpy.log(predicted_values_adjusted)
            + (1 - true_prediction_values) * numpy.log(1 - predicted_values_adjusted)
        )
    ...
    def backward_pass(self, gradient_values, true_prediction_values):
        ...
        self.gradient_inputs = (
            -(
                true_prediction_values / gradient_values_clipped
                - (1 - true_prediction_values) / (1 - gradient_values_clipped)
            )
            / outputs
        )
...
class MeanSquaredErrorLoss(Loss):
    ...
    def forward_pass(self, predicted_values, true_prediction_values):
        ...
        sample_losses = numpy.mean(
            (predicted_values - true_prediction_values) ** 2, axis=-1
        )
        return sample_losses
...
class MeanAbsoluteErrrorLoss(Loss):
    ...
    def forward_pass(self, predicted_values, true_prediction_values):
        ...
        sample_losses = numpy.mean(
            numpy.abs(true_prediction_values - predicted_values), axis=-1
        )
    ...
    def backward_pass(self, gradient_values, true_prediction_values):
        ...
        self.gradient_inputs = (
            numpy.sign(true_prediction_values - gradient_values) / outputs
        )
        self.gradient_inputs = self.gradient_inputs / sample_number
...
class SoftmaxClassifier:  # Currently, we have calculated partial derivatives for both the Categorical Cross Entropy Loss and Softmax Activation functions. But it is actually possible to combine the derivatives of both equations and do it in a, computationally, much faster way. This way, we calculate the combined gradient of the loss and activation functions all in one.
    ...
    def backward_pass(
        ...
        sample_number = len(
            gradient_values
        )  # This finds the number of samples (number of rows of the inputted matrix).

        if (
            len(true_prediction_values.shape) == 2
        ):  # This checks if the vectors are one-hot encoded vectors and, if they are, they are turned into discrete values.
            true_prediction_values = numpy.argmax(true_prediction_values, axis=1)

        self.gradient_inputs = (
            gradient_values.copy()
        )  # A copy of gradient_values must be made in order to ensure that you don't accidentally make alterations to the gradient value array.
        self.gradient_inputs[
            range(sample_number), true_prediction_values
        ] -= 1  # This selects predicted probabilities corresponding to the true class labels for each sample. The minus 1 effectively calculates the gradient of the categorical cross entropy loss with respect to the softmax outputs.
        self.gradient_inputs = (
            self.gradient_inputs / sample_number
        )  # This normalises the gradients by dividing by the number of samples. This ensures the gradients are averaged over the entire batch.
...
class StochasticGradientDescentOptimiser:
    ...
    def update_parameters(self, layer):
        ...
        if (
            self.momentum
        ):  # This will only run if momentum is being implemented into the SGD optimiser.

            if not hasattr(layer, "weight_momentums"):
                layer.weight_momentums = numpy.zeros_like(
                    layer.weights
                )  # If the layer does not contain momentum arrays, create them and fill them with zeroes.
                layer.bias_momentums = numpy.zeros_like(
                    layer.biases
                )  # There needs to be a momentum array for both the weights and the biases of EACH layer.

            weight_updates = (self.momentum * layer.weight_momentums) - (
                self.current_learning_rate * layer.gradient_weights
            )  # Update in the way described in 'logical in-depth explanation of optimisers'
            layer.weight_momentums = weight_updates

            bias_updates = (self.momentum * layer.bias_momentums) - (
                self.current_learning_rate * layer.gradient_biases
            )
...
class AdamOptimiser:
    ...
    def update_parameters(self, layer: numpy.ndarray):
        ...
        layer.weight_cache = (self.BETA_2 * layer.weight_cache) + (
            (1 - self.BETA_2) * layer.gradient_weights**2
        )
        layer.bias_cache = (self.BETA_2 * layer.bias_cache) + (
            (1 - self.BETA_2) * layer.gradient_biases**2
        )
...
class CategoricalAccuracy(Accuracy):
    ...
    def compare(
        self, predicted_values, true_prediction_values
...
class Model:
    ...
    def set_model_parameters(self, parameters):
        for parameter_set, layer in zip(
            parameters, self.trainable_layers
        ):  # Here, zip() takes a list of parameters and a list of layers and returns a new iterator containing 0th elements of both lists in one tuple, the 1st elements of both lists in the next tuple etc. This way, it iterates over parameters and the layers they belong to at the same time.
    ...
    def load_model_parameters(self, path):
        with open(path, "rb") as file:
            self.set_model_parameters(pickle.load(file))
    ...
    def finalise_model(self):
        ...
        for i in range(layer_count):
            if i == 0:
                self.layers[i].prev = (
                    self.input_layer
                )  # .prev and .next are dynamically assigned attributes here to help connections between layers show clearly, which is part of the reason a separate - although small - class was made for input layer.
            ...
            ):  # Essentially checking if it is a trainable layer
                self.trainable_layers.append(self.layers[i])

        if (
            self.loss is not None
        ):  # This checks if the model is being loaded with pre-set parameters, meaning it doesn't need to be trained, hence not needing an optimiser.
            self.loss.remember_trainable_layers(self.trainable_layers)

        if isinstance(self.layers[-1], SoftmaxActivation) and isinstance(
            self.loss, CategoricalCrossEntropyLoss
        ):  # This is checking to see if the softmax activation AND categorical cross entropy classes are being used. If so, the softmax classifier previously developed and described can be used to combine the two.
    ...
    def train_model(
        ...
        if validation_data is not None:
            validation_steps = 1  # Default value for the number of steps for validation is validation data is passed.

            input_data_validation, testing_data_validation = validation_data

        if batch_size is not None:
            train_steps = (
                len(input_data) // batch_size
            )  # This finds the number of batches that the dataset must be split into.

            if train_steps * batch_size < len(
                input_data
            ):  # This checks if there is any leftover data (batch size * number of batches essentially). If there is, an additional batch is added to cover leftover data in the dataset.
                train_steps += 1

            if (
                validation_data is not None
            ):  # This conducts the same process as above for batching for any validation data that is passed in.
                validation_steps = len(input_data_validation) // batch_size

                if validation_steps * batch_size < len(input_data_validation):
                    validation_steps += 1
        ...
        for epoch in range(1, epochs + 1):
            ...
            for step in range(
                train_steps
            ):  # For each batch out of the total number of batches
                if batch_size is None:
                    batch_input_data = input_data
                    batch_testing_data = testing_data
                ...
                output = self.forward_pass(batch_input_data, training=True)

                data_loss, regularisation_loss = self.loss.calculate(
                    output, batch_testing_data, regularisation=True
                )

                loss = data_loss + regularisation_loss

                predictions = self.output_layer_activation.predictions(output)
                accuracy = self.accuracy.calculate(predictions, batch_testing_data)

                self.backward_pass(output, batch_testing_data)
                ...
                if not step % print_step or step == train_steps - 1:
                    print(
                        f"step: {step}, "
                        + f"acc: {accuracy:.3f}, "
                        + f"loss: {loss:.3f}, ("
            ...
            epoch_data_loss, epoch_regularisation_loss = (
                self.loss.calculate_accumulated(regularisation=True)
            )
            ...
            if validation_data is not None:
                ...
                )  # The *validation_data unpacks the validation_data list into singular values (e.g. tuple = (1,2) *tuple = 1 2)
    ...
    def backward_pass(self, output, testing_data):
        ...
        if (
            self.softmax_classifier_output is not None
        ):  # Need to check if the softmax classifier is being used in the model

            self.softmax_classifier_output.backward_pass(output, testing_data)

            self.layers[-1].gradient_inputs = (
                self.softmax_classifier_output.gradient_inputs
            )
    ...
    def evaluate_model(
        self, input_data_validation, testing_data_validation, batch_size=None
    ):
        validation_steps = 1
        ...
        for step in range(validation_steps):
            if batch_size is None:
                batch_input_data = input_data_validation
                batch_testing_data = testing_data_validation
            ...
            output = self.forward_pass(batch_input_data, training=False)

            self.loss.calculate(output, batch_testing_data)

            predictions = self.output_layer_activation.predictions(output)

            self.accuracy.calculate(predictions, batch_testing_data)
    ...
    def predict(self, input_data, batch_size=None):
        ...
        if batch_size is not None:
            prediction_steps = len(input_data) // batch_size

            if prediction_steps * batch_size < len(input_data):
                prediction_steps += 1
        ...
        for step in range(prediction_steps):
            ...
            batch_output = self.forward_pass(batch_input_data, training=False)
...
def load_dataset(dataset, path):
    ...
    images = (
        []
    )  # This is defined as a list initially, and will have images represented as numpy arrays added to it.
    target_labels = []
    ...
    for label in labels:
        ...
        for file in os.listdir(os.path.join(path, dataset, label)):
            ...
            image = cv2.imread(
                os.path.join(path, dataset, label, file), cv2.IMREAD_GRAYSCALE
            )
    ...
    return numpy.array(images), numpy.array(target_labels).astype(
        "uint8"
    )  # Since images has been defined as a list, and images are being added to the list in the form of Numpy arrays, need to call numpy.array on both images and labels to transform them both into numpy arrays. Had problems with label integers, 'uint8' informs numpy that they are all integers.
