...
class SoftmaxActivation: 
    ...
    def forward_pass(self, inputs: numpy.ndarray): 
        """
        ...
        for row in inputs: 
            ...
            for i in range(len(row)): 
                row[i] = row[i] - max_value #This essentially subtracts the max value of each row of the inputs. As explored in more detail in notebook, this prevents dead neurons and exploding values. 

        exp_values = numpy.exp(inputs) 
...
class CategoricalCrossEntropyLoss(Loss): 
    ...
    def backward_pass(self, predicted_values: numpy.ndarray, true_prediction_values: numpy.ndarray): 
        """
        ...
        """
        samples = len(predicted_values) #This finds the number of samples (as this is the number of rows of the predicted_values matrix). 
        labels = len(predicted_values[0]) #This finds the number of classes (finding the number of columns in the first row as the number of output values/classes should be the same for each sample). 
        ...
        self.gradients = -(true_prediction_values)/predicted_values #This calculates the gradient based on the derivative of the categorical cross entropy loss function, using the relevant mathematical equation. 
        self.gradients = self.gradients/samples #This noramlises the gradient based on the number of samples 
...
class BinaryCrossEntropyLoss(Loss): 
    ...
    def forward_pass(self, predicted_values, true_prediction_values): 
        ...
        predicted_values_clipped = numpy.clip(predicted_values, 1e-7, 1 - 1e-7)

        sample_losses = -(true_prediction_values * numpy.log(predicted_values_clipped) + (1 - true_prediction_values) * numpy.log(1 - predicted_values_clipped))
...
class Accuracy: 
    ...
    def calculate(self, predicted_values, true_prediction_values): 
        ...
        self.accumulated_sum += numpy.sum(comparisons) #This accumulation is for when you're training with batches. 
    ...
    def new_pass(self): #As in the loss new_pass, the accumulated_sum and accumulated_count values need to be reset at the beginning of each epoch.
        self.accumulated_sum = 0 
        self.accumulated_count = 0
...
class Model: 
    ...
    def train_model(self, input_data, testing_data, epochs=1, batch_size=None, print_step=1, validation_data=None): 
...
#TRAINING NOTES: 
#Stuck in local minimum (loss accuracy and loss stuck pretty high): increase learning rate and add learning rate decay 
